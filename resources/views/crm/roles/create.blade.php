...
@section('title', __('إضافة دور جديد') . ' | AutoCRM')
...
<div class="container-fluid" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}">
    ...
    <div class="d-flex align-items-center justify-content-between mb-4">
        <div>
            <h4 class="mb-1 fw-bold">🛡️ {{ __('إضافة دور جديد') }}</h4>
            <p class="text-muted mb-0 small">{{ __('قم بإضافة دور جديد وتحديد صلاحياته') }}</p>
        </div>
        <div>
    ...
    <div class="card border-0 shadow-sm rounded-4 overflow-hidden">
        <div class="card-body p-4">
            <form action="{{ route('crm.roles.store') }}" method="POST">
                ...
                <div class="mb-4">
                    <label class="form-label fw-bold">{{ __('اسم الدور (مثل: مدير مبيعات)') }} <span class="text-danger">*</span></label>
                    <input type="text" name="name" class="form-control form-control-lg bg-light border-0" value="{{ old('name') }}" required placeholder="{{ __('أدخل اسم الدور') }}">
                </div>
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-3 col-sm-6">
                            <div class="form-check form-switch p-3 border rounded-3 bg-light">
                                <input class="form-check-input float-end ms-0 me-2" type="checkbox" name="permissions[]" value="{{ $permission->name }}" id="perm_{{ $permission->id }}" {{ in_array($permission->name, old('permissions', [])) ? 'checked' : '' }}>
                ...
                <div class="d-flex justify-content-end mt-4">
                    ...
                    <button type="submit" class="btn btn-primary rounded-pill px-5 py-2 shadow-sm fw-bold">
                        <i class="bi bi-check2-circle me-1"></i> {{ __('حفظ الدور') }}
                    </button>
                    @endcan
                </div>
            </form>
